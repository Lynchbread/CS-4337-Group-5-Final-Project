...
def identify_objects(past_frame, current_frame, future_frame):
    ...
    lower_brown = np.array([0, 25, 50], dtype=np.uint8)
    upper_brown = np.array([30, 60, 128], dtype=np.uint8)
    #lower_brown = np.array([0, 25, 50], dtype=np.uint8)
    #upper_brown = np.array([40, 80, 150], dtype=np.uint8)
    brown_mask = cv2.inRange(hsv_frame, lower_brown, upper_brown)

    # For testing. Delete later
    if (identify_objects.call == 250):
        brown_extracted = cv2.bitwise_and(current_frame, current_frame, mask=brown_mask)
    ...
    threshold, binary_image = cv2.threshold(motion, thresh = 10, maxval = 255, type = cv2.THRESH_BINARY)

    binary_image = cv2.bitwise_and(binary_image, binary_image, mask=brown_mask)

    binary_image = cv2.erode(binary_image, np.ones((3, 3), np.uint8), iterations=1)
    binary_image = cv2.dilate(binary_image, np.ones((3, 3), np.uint8), iterations=7)

    # For testing. Delete later
    if (identify_objects.call == 249):
    ...
    #new_image = cv2.Mat.copy(current_frame)
    rectangles = []
